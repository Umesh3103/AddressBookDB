...
public class AddressBookServiceTest {
	...
	public void givenDateRange_WhenRetrieved_ShouldMatchContactCount() {
		...
		try {
			...
			List<Details> addressBookList = addressBookService.readAddressBookForDateRange(IOService.DB_IO, startDate,
					endDate);
	...
	public void givenAddressBookDB_WhenRetrievedByCityCount_ShouldReturnProperValue() {
		...
		try {
			...
			Assert.assertTrue(countOfCityByState.get("Rajasthan").equals(4) && countOfCityByState.get("UP").equals(1)
					&& countOfCityByState.get("Bihar").equals(1));
		} catch (AddressBookException e) {
		}
	}
	...
	public void givenNewContact_WhenAdded_ShouldSyncWithDB() {
		...
		try {
			addressBookService.readAddressBookDB(IOService.DB_IO);
			addressBookService.addContactsToAddressBook("Abhinav", "Singh", 9983917564L, "abh@gmail.com",
					LocalDate.now());
	...
	public void givenMultipleContacts_WhenAdded_ShouldMatchAddressBookEntries() {
		Details[] arrayOfContacts = { new Details("Jofra", "Archer", 7538964120L, "jfh@gmail.com", LocalDate.now()),
	...
	@Test
	public void givenListOfPersons_WhenAdded_ShouldMatch201ResponseCount() {
		Details[] arrayOfContacts = getContactsList();
		AddressBookService addressBookService;
		addressBookService = new AddressBookService(Arrays.asList(arrayOfContacts));
		Details[] contactDetails = { new Details(4, "Sanju", "Samson", 4567891230L, "sms@gmail.com", LocalDate.now()),
				new Details(5, "Ben", "Stokes", 1478523690L, "bst@gmail.com", LocalDate.now()),
				new Details(6, "Jofra", "Archer", 1598746320L, "jfh@gmail.com", LocalDate.now()) };
		for (Details data : contactDetails) {
			Response response = addContactsToJsonServer(data);
			int result = response.getStatusCode();
			Assert.assertEquals(201, result);
			data = new Gson().fromJson(response.asString(), Details.class);
			addressBookService.addContactToAddressBook(data, IOService.REST_IO);
		}
		long entries = addressBookService.countEntries();
